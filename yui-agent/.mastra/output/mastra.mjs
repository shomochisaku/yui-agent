...
const webSearchTool = createTool({
  ...
  execute: async ({ query, maxResults = 5 }) => {
    try {
      console.log(`\u{1F50D} Web\u691C\u7D22\u3092\u5B9F\u884C: "${query}"`);
      ...
      if (data.web && data.web.results && data.web.results.length > 0) {
        data.web.results.slice(0, maxResults).forEach((item) => {
          results.push({
...
export { Agent as A, MastraError as M, Tool as T, ZodFirstPartyTypeKind as Z, Telemetry as a, AvailableHooks as b, checkEvalStorageFields as c, TABLE_EVALS as d, executeHook as e, createTool as f, isVercelTool as i, mastra as m, registerHook as r, webSearchTool as w, z };
